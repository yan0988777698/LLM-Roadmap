...
- [ ] 能解釋 `ndarray` 的 `shape`、`ndim`、`size`、`dtype`。
- [ ] 能預測切片的 shape，並在元素總數不變下 reshape。
- [ ] 能說明 `*` 與 `@`，並各自算出一個小例子。
- [ ] 先手算 `X @ W + b` 的 shape 與數值，再完成 `practice.py` 的檢查。
- [ ] 能由 `(4, 3) @ (3, 2) + (2,)` 推出輸出 shape，並說明 batch 改變的影響。
- [ ] 找出一個 broadcasting 錯誤，解釋不相容的軸與正確偏移的意義。
- [ ] 在進度紀錄填實際用時、驗收結果，區分跟做與獨立完成。

## 學習筆記（自行填寫）

- 實際日期／概念、實作除錯、整理用時：待填。
- 我預測的 shape 與實際結果：待填。
- 我遇到的 broadcasting 問題與原因：待填。
- 不看示範也能解釋的部分：待填。
- 尚未理解：待填。
